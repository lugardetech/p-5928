...
async function fetchAllClosedClaims(accessToken: string) {
  const claims = [];
  let offset = 0;
  const limit = 50;
  let hasMore = true;
  ...
  while (hasMore) {
    console.log(`🔄 Buscando reclamações fechadas com offset ${offset}...`);
    
    const response = await fetch(
      `https://api.mercadolibre.com/post-purchase/v1/claims/search?status=closed&limit=${limit}&offset=${offset}`,
      {
        headers: {
    ...
    console.log(`✅ Recebidas ${data.data.length} reclamações fechadas`);
...
serve(async (req) => {
  ...
  try {
    ...
    // Buscar todas as reclamações fechadas na API do Mercado Livre
    const claims = await fetchAllClosedClaims(integration.access_token);
    console.log(`✅ Total de ${claims.length} reclamações fechadas obtidas`);
    ...
    for (const claim of claims) {
      ...
      if (upsertError) {
        console.error("❌ Erro ao salvar reclamação fechada:", upsertError);
      }
    }

    // Buscar reclamações fechadas atualizadas do banco
    ...
    if (dbError) {
      console.error("❌ Erro ao buscar reclamações fechadas do banco:", dbError);
      throw dbError;
    }
